...
# =========================
# Average-zone ARMAX (scalar)
# =========================
def compute_avg_armax(armax_config):
    """
    Build average-zone (scalar) ARMAX parameters from a multi-zone model.
    Produces scalars A_l, B_l, c_l and vector D_l (length = solar_terms) for each lag,
    plus a scalar bias b. All are independent of n_zones.
    """
    coef         = armax_config["armax_coef"]      # shape: (n_features, n_zones)
    ...
    intercept    = armax_config["armax_intercept"].reshape(-1)  # (n_z,)

    Abar, Bbar, cbar, Dbar = {}, {}, {}, {}
    for i, lag in enumerate(lags):
        idx  = i * feat_length
        Ablk = coef[idx + pos_T1 : idx + pos_T1 + n_z, :]              # (n_z, n_z)
        Bblk = coef[idx + pos_u1 : idx + pos_u1 + n_z, :]              # (n_z, n_z)
        cblk = coef[idx + pos_T_amb, :]                                # (n_z,)
        Dblk = coef[idx + pos_s1 : idx + pos_s1 + solar_terms, :]      # (n_solar, n_z)

        # Equal-weight average-zone scalarization:
        #  A_bar = (1/nz) * 1^T A 1,  B_bar = (1/nz) * 1^T B 1
        Abar[lag] = Ablk.sum() / n_z
        Bbar[lag] = Bblk.sum() / n_z
        # ambient and solar averaged over zones:
        cbar[lag] = cblk.sum() / n_z
        # D_bar is 1 x n_solar (average over zones for each bin)
        Dbar[lag] = (Dblk.sum(axis=1) / n_z)  # shape: (n_solar,)

    bbar = intercept.mean()

    return {
        "A": Abar, "B": Bbar, "c": cbar, "D": Dbar, "b": bbar,
        "lags": lags, "solar_terms": solar_terms
    }


def make_scalar_armax_config(avg_params, history_length, horizon_length):
    """
    Pack into a config object analogous to your multi-zone armax_config,
    but marked as single-zone (scalar) model.
    """
    return {
        "model_type": "ARMAX_scalar",
        "avg_params": avg_params,                   # dict with scalars per lag
        "history_length": history_length,
        "horizon_length": horizon_length,
        "solar_terms": avg_params["solar_terms"],
        "dynamics_lags_list": avg_params["lags"]
    }
