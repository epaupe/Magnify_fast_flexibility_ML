...
ITERATION_NAME = "best_flex_fusion_CNN_MLP_new_dataset"
...
def get_data(
  ...
  # ==========================================================
    ...
    X_static_train_raw = torch.stack([st for _, st, _ in train_set])   # (N_train,85)
    ...
    static_mean = X_static_train_raw.mean(dim=0, keepdim=True) # (1,85)
    static_std  = X_static_train_raw.std(dim=0, keepdim=True)  # (1,85)
    ...
    def normalize_subset(subset):
        ...
        for idx in subset.indices:        # iterate over indices in the Subset
            ...
            static_mean_1d = static_mean.view(85)
            static_std_1d  = static_std.view(85)
